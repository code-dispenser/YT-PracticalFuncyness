...
internal sealed record class Validated<T> where T : notnull
{
    ...
    {
        var filteredFailures = failures?.Where(failure => !String.IsNullOrEmpty(failure.FieldName) && !String.IsNullOrEmpty(failure.Message)).ToList() ?? [];
